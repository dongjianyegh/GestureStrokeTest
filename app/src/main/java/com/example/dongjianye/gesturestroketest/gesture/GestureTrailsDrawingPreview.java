...
public final class GestureTrailsDrawingPreview extends AbstractDrawingPreview implements Runnable {
    ...
    public GestureTrailsDrawingPreview(final GestureTrailDrawingParams drawingParams) {
    ...
    public void setKeyboardViewGeometry(final int[] originCoords, final int width,
        ...
        mOffscreenOffsetY = 500; // 可以设置成toolbar的高度
    ...
    public void drawPreview(final Canvas canvas) {
        ...
        if (!mDirtyRect.isEmpty()) {
            ...
            canvas.save();
            canvas.clipRect(0, -mOffscreenOffsetY, mOffscreenWidth, mOffscreenHeight - mOffscreenOffsetY);
    ...
    @Override
    public void setPreviewPosition(final GestureStrokeDrawingPoints points,
        ...
        synchronized (mGestureTrails) {
            trail = mGestureTrails.get(pointerId);
            if (trail == null) {
                trail = new GestureTrailDrawingPoints();
                mGestureTrails.put(pointerId, trail);
            }
        }
        trail.addStroke(points, downTime);
